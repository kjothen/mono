(ns ^:eftest/synchronized
 com.repldriven.mono.transactions.interface-test
  (:require
    com.repldriven.mono.transactions.interface
...
(defn- test-unknown-command
  ...
  (testing "unknown command returns rejection"
    (let [result
      ...
      (is (= :transactions/unknown-command
             (error/kind result))))))

(deftest process-transaction-test
  (with-test-system
    [sys "classpath:transactions/application-test.yml"]
